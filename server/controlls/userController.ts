...
//  Get user  Creadits
export const getUserCreadits = async (req: Request, res: Response) => {
    try {
...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(user && user.credits < 5){
            return res.status(403).json({ message: 'add creadits to create more projects'});
        }
...
export const TogglePublish = async (req: Request, res: Response) => {
    try {
       ...
       await prisma.websiteProject.update({
        ...
        res.json({message: project.isPublished ? 'Project Unpublished' : 'Project  Published  Succefully'})
